...
from APQ.elastic_nn.modules.dynamic_layers import DynamicMBConvLayer, DynamicConvLayer, DynamicLinearLayer, \
    DynamicMBQConvLayer, DynamicQConvLayer, DynamicQLinearLayer
from APQ.imagenet_codebase.modules.layers import ConvLayer, IdentityLayer, LinearLayer, MBInvertedConvLayer, \
    MBInvertedQConvLayer, \
    QConvLayer, QLinearLayer
from APQ.imagenet_codebase.networks.proxyless_nets import ProxylessNASNets, MobileInvertedResidualBlock, AvgMBConvStage
from APQ.imagenet_codebase.utils import make_divisible, int2list, list_weighted_sum, AverageMeter
...
class DynamicQuantizedProxylessNASNets(ProxylessNASNets):
    ...
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-3), dropout_rate=0.1, base_stage_width=None,
        ...
        #print('len(last_channel)',len(last_channel))
        #print('max(input_channel)', max(input_channel))
        #print('max(last_channel)', max(last_channel))
        if len(last_channel) == 1:
            ...
            #print('feature_mix_layer',feature_mix_layer)
    ...
    def forward(self, x):
        ...
        for stage_id, (block_idx, _) in enumerate(self.block_group_info):
            #print('self.block_group_info',self.block_group_info)
        ...
        #print('x.shape1: ',x.mean(3).mean(2).shape)
    ...
    def set_active_subnet(self, wid=None, ks=None, e=None, d=None):
       ...
       # print(depth)
        ...
        for i, d in enumerate(depth):
            if d is not None:
                #print('len(self.block_group_info[i][0])')
                #print(len(self.block_group_info[i][0]))
                #print('')
                #print('runtime_depth[i]')
                #print(self.runtime_depth[i])
                #print('')
