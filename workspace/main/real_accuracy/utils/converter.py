...
class Converter():
    ...
    def random_spec(self):
        ...
        #print(self.e_info)
        for i in range(21):
            #print(random.sample(self.k_info['val2id'][i].keys(), 1))
            #print(i)
            #print(self.k_info['val2id'][i].keys())
            #print(self.e_info['val2id'][i].keys())
            #print(len(self.e_info['val2id'][i].keys()))
            #print(random.sample(self.k_info['val2id'][i].keys(), 1)[0])
            #print('')
            info['ks'].append(random.sample(self.k_info['val2id'][i].keys(), 1)[0])
            info['e'].append(random.sample(self.e_info['val2id'][i].keys(), 1)[0])
            for it in range(4):
                #print(self.num2qname[it])
                #print(random.sample(self.q_info[it]['val2id'][i].keys(), 1)[0])
                info[self.num2qname[it]].append(random.sample(self.q_info[it]['val2id'][i].keys(), 1)[0])

        return info
    
    def random_spec_setSampled(self, d_sam, ks_sam, e_sam, q_pw_w_sam, q_pw_a_sam, q_dw_w_sam, q_dw_a_sam):
        info = {'wid': None, 'ks': [],
                'e': [], 'd': [], 'pw_w_bits_setting': [], 'pw_a_bits_setting': [], 'dw_w_bits_setting': [],
                'dw_a_bits_setting': []}
        
        # these are already lists of values!! --> we're just organizing them ot the dictionaries
        #d_sam, ks_sam, e_sam, q_pw_w_sam, q_pw_a_sam, q_dw_w_sam, q_dw_a_sam
        for i in range(6):
            #print(np.random.randint(3) + 2)
            info['d'].append(d_sam[i])
            #info['d'].append(np.random.randint(3) + 2)
            #print()
        ...
        for i in range(21):
            ...
            info['ks'].append(ks_sam[i])
            info['e'].append(e_sam[i])
            ...
            for it in range(4):
                ...
                #info[self.num2qname[it]].append(random.sample(self.q_info[it]['val2id'][i].keys(), 1)[0])
                if it==0:
                    info[self.num2qname[it]].append(q_pw_w_sam[i])
                elif it==1:
                    info[self.num2qname[it]].append(q_pw_a_sam[i])
                elif it==2:
                    info[self.num2qname[it]].append(q_dw_w_sam[i])
                elif it==3:
                    info[self.num2qname[it]].append(q_dw_a_sam[i])
